...
public class SUB extends Frame implements ActionListener {
	...
	public static void RANA() {
		if (Mainbar.sasc < ExButton.quizAA) {
			...
			System.out.println("랭크 갱신 " + Mainbar.sasc + " 점");
	...
	public SUB() {
		...
		open = new Button("당신의 정답 수는 " + ExButton.quizAA + " 개 입니다."); // 메인에서 점수 가져와서 출력.
		System.out.println("당신의 정답 수는 " + ExButton.quizAA + " 개 입니다."); // 디버깅
		open.setFont(new Font("맑은고딕", Font.BOLD, 30)); // 버튼에 있는 글씨의 폰트
		open.setCursor(new Cursor(Cursor.HAND_CURSOR)); // 커서 갖다대면 커서 외형이 변경됨
		// 점수에 따라 버튼 색상 변경.
		if (ExButton.quizAA <= 20) { // 150 점 이하는 이것으로 처리.
			open.setBackground(new Color(255, 50, 0)); // 낮은점수 빨강색 배경 설정
		} // 배경설정 끝

		if (ExButton.quizAA > 20 && ExButton.quizAA <= 40) { // 151점 이상 500 점 이하는 이것으로 처리.
